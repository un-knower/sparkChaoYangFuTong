...
def sum_entrance_tmp(timedate):
    ...
    args = []
    for row in results:
        sql_data = ([row[0], today, row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], update_at_tmp])
        args.append(sql_data)
    try:
        cur.executemany(sql2, args)
    except Exception, e:
...
def sum_page_tmp(timedate):
    update_at_tmp = int(time.time())
    (today, seven_days_ago, update_at) = seven_time(timedate)
    sql = "SELECT app_key,page_path,SUM(page_count),SUM(open_count),SUM(total_stay_time),SUM(abort_page_count),SUM(share_count),SUM(total_stay_time)/SUM(open_count),SUM(abort_page_count)/SUM(page_count) FROM aldstat_page_view where day >=" + "'" + seven_days_ago + "'" + " and day <=" + "'" + today + "'" + " GROUP BY app_key,page_path"
    conn = db_connect()
    cur = conn.cursor()
    cur.execute(sql)
    results = cur.fetchall()
