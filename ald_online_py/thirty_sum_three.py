...
def sum_phone_brand_tmp(timedate):
    ...
    sql = "SELECT app_key,brand,SUM(new_user_count),SUM(open_count),SUM(page_count),SUM(total_stay_time)," \
          ...
          "FROM aldstat_daily_phonebrand where day >=" + "'" + thirtydays_ago_str + "'" + " and day <=" + "'" + yesterday_str + "'" + " GROUP BY app_key,brand"
    conn = db_connect()
    cur = conn.cursor()
    ...
    try:
        cur.executemany(sql_30day_pb, args)  # 批量入库
    except Exception, e:
...
def sum_phone_model_tmp(timedate):
    ...
    # 将30日汇总结果写入 30 日结果表中
    sql_30day_device = """
        insert into ald_30days_device_statistics (app_key,date,phone_model,new_user_count,
        open_count,page_count,total_stay_time,one_page_count,bounce_rate,secondary_stay_time,update_at)
        values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
    ...
    for row in results:
        ...
        args.append(sql_data)
    try:
        cur.executemany(sql_30day_device, args)
    except Exception, e:
        print Exception, ":", e, ' sql insert error'
    # 关闭数据库连接
    cur.close()
...
def sum_entrance_tmp(timedate):
    ...
    sql = "SELECT app_key,page_path,SUM(entry_page_count),SUM(one_page_count),SUM(page_count)," \
          ...
          "SUM(one_page_count)/SUM(page_count) FROM aldstat_entrance_page where " \
          "day >=" + "'" + thirtydays_ago_str + "'" + " and day <=" + "'" + yesterday_str + "'" \
          + " GROUP BY app_key,page_path"
    conn = db_connect()
    cur = conn.cursor()
    cur.execute(sql)  # 查询各项指标并计算次均停留时长和跳出率
    results = cur.fetchall()
    ...
    sql_30days_single = """
        ...
        values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
        ON DUPLICATE KEY UPDATE entry_page_count=VALUES(entry_page_count),one_page_count=VALUES(one_page_count),
        page_count=VALUES(page_count),open_count=VALUES(open_count),total_time=VALUES(total_time),
        update_at=VALUES(update_at) ,avg_stay_time=VALUES(avg_stay_time),bounce_rate=VALUES(bounce_rate)"""
    # 批量写入
    args = []
    for row in results:
        sst = 0  # 次均停留时长
        ...
        args.append(sql_data)
    try:
        cur.executemany(sql_30days_single, args)  # 批量入库单个场景值入口页
    except Exception, e:
        print Exception, ":", e, ' sql insert error'
    # 关闭数据库连接
    cur.close()
...
def sum_page_tmp(timedate):
    ...
    #     获取30天的开始和结束日期
    (yesterday_str, thirtydays_ago_str, update_at) = thirty_time(timedate)
    sql = "SELECT app_key,page_path,SUM(page_count),SUM(open_count),SUM(total_time)," \
          "SUM(abort_page_count),SUM(share_count),SUM(total_time)/SUM(page_count)," \
    ...
    # 将30日汇总结果写入 30 日结果表中abort_ratio
    sql_single_page = """
    ...
    for row in results:
        ...
        args.append(sql_data)
    try:
        cur.executemany(sql_single_page, args)  # 批量入库单个受访页
    except Exception, e:
        print Exception, ":", e, ' sql insert error'
    # 关闭数据库连接
    cur.close()
...
def sum_province_tmp(timedate):
    ...
    for row in results:
        ...
        args.append(sql_data)
    try:
        cur.executemany(sql_province, args)
    except Exception, e:
        print Exception, ":", e, ' sql insert error'
    # 关闭数据库连接
    cur.close()
...
def sum_city_tmp(timedate):
    ...
    # 将30日汇总结果写入 30 日结果表中
    sql_city = """
        insert into aldstat_30days_city_statistics (app_key,day,city,new_user_count,open_count,page_count,
        total_stay_time,one_page_count,secondary_stay_time,bounce_rate,update_at)
        values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
        ON DUPLICATE KEY UPDATE new_user_count=VALUES(new_user_count),open_count=VALUES(open_count),
        page_count=VALUES(page_count),total_stay_time=VALUES(total_stay_time),
    ...
    for row in results:
        ...
        args.append(sql_data)
    try:
        cur.executemany(sql_city, args)  # 批量入库地域（安市）
    except Exception, e:
        print Exception, ":", e, ' sql insert error'
    # 关闭数据库连接
    cur.close()
...
def sum_duration_tmp(timedate):
    ...
    #     获取30天的开始和结束日期
    (yesterday_str, thirtydays_ago_str, update_at) = thirty_time(timedate)
    sql = "SELECT a.app_key,a.visit_duration,a.one_count,a.one_count/b.day_sum_count from " \
          "(SELECT app_key,visit_duration,SUM(open_count) one_count from aldstat_visit_duration where" \
          " day >=" + "'" + thirtydays_ago_str + "'" + " and day <=" + "'" + yesterday_str + "'" \
          + " GROUP BY app_key,visit_duration) a LEFT JOIN (SELECT app_key,SUM(open_count) " \
            "day_sum_count from aldstat_visit_duration where day >=" + "'" + thirtydays_ago_str \
          + "'" + " and day <=" + "'" + yesterday_str + "'" + " GROUP BY app_key) b on a.app_key=b.app_key"
    conn = db_connect()
    cur = conn.cursor()
    cur.execute(sql)  # 查询各项指标准备入库
    results = cur.fetchall()
    # 将30日汇总结果写入 30 日结果表中
    sql_visit_duration = """
    ...
    values (%s,%s,%s,%s,%s,%s)
    ON DUPLICATE KEY UPDATE open_ratio=VALUES(open_ratio),open_count=VALUES(open_count),
    update_at=VALUES(update_at) """
    # 批量写入
    args = []
    # 循环遍历results挑选入库的各项指标并存入批量入库列表中
    for row in results:
...
def sum_depth_tmp(timedate):
    ...
    #     获取30天的开始和结束日期
    (yesterday_str, thirtydays_ago_str, update_at) = thirty_time(timedate)
    sql = "SELECT a.app_key,a.visit_depth,a.one_count,a.one_count/b.sum_count from " \
          "(SELECT app_key,visit_depth,SUM(open_count) one_count from aldstat_visit_depth where " \
          "day >=" + "'" + thirtydays_ago_str + "'" + " and day <=" + "'" + yesterday_str + "'" + " GROUP BY " \
                                                                                                  "app_key,visit_depth) a LEFT JOIN (SELECT app_key,SUM(open_count) sum_count from " \
                                                                                                  "aldstat_visit_depth where day >=" + "'" + thirtydays_ago_str + "'" + " and day <=" \
          + "'" + yesterday_str + "'" + " GROUP BY app_key) b on a.app_key = b.app_key"
    conn = db_connect()
    cur = conn.cursor()
    cur.execute(sql)  # 查询入库的各项指标
    results = cur.fetchall()
    # 将30日汇总结果写入 30 日结果表中
    sql_visit_depth = """
    ...
    for row in results:
        ...
        args.append(sql_data)
    try:
        cur.executemany(sql_visit_depth, args)  # 批量入库访问深度表中
    except Exception, e:
        print Exception, ":", e, ' sql insert error'
    # 关闭数据库连接
    cur.close()
...
def sum_entrance_all(timedate):
    ...
    sql_entrance_page = """
           ...
           (app_key,day,entry_page_count,one_page_count,page_count,open_count,total_time,
           avg_stay_time,bounce_rate,update_at) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
...
def sum_page_view_all(timedate):
    ...
    sql = "SELECT app_key,SUM(page_count),SUM(open_count),SUM(total_time),SUM(abort_page_count)," \
          "SUM(share_count),SUM(total_time)/SUM(page_count),SUM(abort_page_count)/SUM(page_count) FROM " \
          "aldstat_daily_page_view where day >=" + "'" + thirtydays_ago_str + "'" + " and day <=" \
          + "'" + yesterday_str + "'" + " GROUP BY app_key"
    conn = db_connect()
    cur = conn.cursor()
    cur.execute(sql)  # 查询入库的各项指标
    results = cur.fetchall()
    ...
    try:
        ...
        print Exception, ":", e, ' sql insert error', args
    # 关闭数据库连接
    cur.close()
    conn.commit()
    conn.close()
