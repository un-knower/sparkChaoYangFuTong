...
object ATStat  {
   ...
   val name ="EventAT"  //gcs:AT的计算时不用进行去重的操作。所以就没有必要使用Redis的去重的操作了
...
//          if (1 == insertCount2) {
          ...
          if (StringUtils.isNotBlank(recordTp)){
            try{
...
//          }
       ...
       .mapPartitions(record =>{
         ...
         record.foreach(str =>{
           ...
           var baseKeyFieldUU =s"${str._1}:at"
