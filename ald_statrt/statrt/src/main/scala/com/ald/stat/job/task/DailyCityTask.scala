...
object DailyCityTask extends TaskTrait {
  ...
  def dailyStat(baseRedisKey: String, taskId: String, dateStr: String, logRecordRdd: RDD[LogRecord], kafkaProducer: Broadcast[KafkaSink[String, String]], redisPrefix:Broadcast[String]):  Unit = {
    val tempSessMergeResult = SessionStatImpl.stat(logRecordRdd, DailyCitySessionSubDimensionKey, SessionBaseImpl)
    //session处理
    val sessionRDD = SessionStatImpl.doCache(baseRedisKey, taskId, dateStr, tempSessMergeResult, DailyCitySessionSubDimensionKey, redisPrefix)
    //uv
    val uvRDD = UVStat.statIncreaseCache(baseRedisKey, dateStr, DailyCityUidSubDimensionKey, logRecordRdd, redisPrefix)
    //open count
    val opRDD = UVStat.statIncreaseCache(baseRedisKey, dateStr, DailyCitySessionSubDimensionKey, logRecordRdd, redisPrefix)
    //pv
    val pvRDD = PVStat.statIncreaseCache(baseRedisKey, dateStr, taskId, DailyCityDimensionKey, logRecordRdd, redisPrefix)
