...
object AnalysisScene {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sceneRedisKey = "rt:scene"

    val resource = redisCache.getResource
    val sceneMap = resource.hgetAll(sceneRedisKey)
              ...
              statement =>
                ...
                while (rs.next()) {
                  ...
                  if (sid != null && sgid != null) {
                    ...
                    resource.hset(sceneRedisKey, sid.toString, sgid.toString)
                  }
                }
            }
          }
    ...
    stream.repartition(partitionNumber).foreachRDD(rdd => {
      val logRecordRdd: RDD[LogRecord] = rdd.map(line => {
        ...
        if (StringUtils.isBlank(logRecord.scene)) {
          ...
          val sgid = broadcastSceneMap.value.get(logRecord.scene.trim)
      ...
      HourlySceneTask.hourlyGroupStat(baseRedisKey, taskId, dateStr, logRecordRdd, kafkaProducer, redisPrefix)
      DailySceneTask.dailyStat(baseRedisKey, taskId, dateStr, logRecordRdd, kafkaProducer, redisPrefix)
      DailySceneTask.dailyGroupStat(baseRedisKey, taskId, dateStr, logRecordRdd, kafkaProducer, redisPrefix)
    })
    ssc.start()
