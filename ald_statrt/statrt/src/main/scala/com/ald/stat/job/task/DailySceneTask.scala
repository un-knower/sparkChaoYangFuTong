...
object DailySceneTask extends TaskTrait {
  ...
  def dailyStat(baseRedisKey: String, taskId: String, dateStr: String, logRecordRdd: RDD[LogRecord], kafkaProducer: Broadcast[KafkaSink[String, String]], redisPrefix: Broadcast[String]): Unit = {
    val tempSessMergeResult = SessionStatImpl.stat(logRecordRdd, DailySceneSessionSubDimensionKey, SessionBaseImpl)
    //session处理
    val sessionRDD = SessionStatImpl.doCache(baseRedisKey, taskId, dateStr, tempSessMergeResult, DailySceneSessionSubDimensionKey, redisPrefix)
    //uv
    val uvRDD = UVStat.statIncreaseCacheWithPV(baseRedisKey, dateStr, DailySceneUidSubDimensionKey, logRecordRdd, redisPrefix)
    //open count
    val opRDD = UVStat.statIncreaseCache(baseRedisKey, dateStr, DailySceneSessionSubDimensionKey, logRecordRdd, redisPrefix)
      ...
      par.foreach(row => {
        ...
        val sqlInsertOrUpdate =
          s"""
             ...
             |day="$day",
             |scene_id="$sceneId",
             |scene_newer_for_app=scene_newer_for_app+"$new_comer_count",
             |scene_visitor_count=scene_visitor_count+"$visitor_count",
             |scene_open_count=scene_open_count+"$open_count",
             |scene_page_count=scene_page_count+"$page_count",
  ...
  /**
    ...
    * @param redisPrefix
    */
  def dailyGroupStat(baseRedisKey: String, taskId: String, dateStr: String, logRecordRdd: RDD[LogRecord], kafkaProducer: Broadcast[KafkaSink[String, String]], redisPrefix: Broadcast[String]): Unit = {
    val tempSessMergeResult = SessionStatImpl.stat(logRecordRdd, DailySceneGroupSessionSubDimensionKey, SessionBaseImpl)
    //session处理
    val sessionRDD = SessionStatImpl.doCache(baseRedisKey, taskId, dateStr, tempSessMergeResult, DailySceneGroupSessionSubDimensionKey, redisPrefix)
    //uv
    val uvRDD = UVStat.statIncreaseCacheWithPV(baseRedisKey, dateStr, DailySceneGroupUidSubDimensionKey, logRecordRdd, redisPrefix)
    //open count
    val opRDD = UVStat.statIncreaseCache(baseRedisKey, dateStr, DailySceneGroupSessionSubDimensionKey, logRecordRdd, redisPrefix)
