...
object AnalysisLink {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val linkNewUserRedisKey = "rt:linkNewUser"
          ...
          resource =>
            par.foreach(line => {
              ...
              if (logRecord != null && logRecord.ifo == "true") {
                resource.hset(linkNewUserRedisKey, logRecord.at, "true")
              }
            })
        }
      })
      ...
      .foreachRDD(rdd => {
        val logRecordRdd = rdd.mapPartitions(
          par => {
            ...
            //广播变量
            //            val mediaMap = ssc.sparkContext.broadcast(linkMap)
              ...
              resource =>
                par.foreach(line => {
                  ...
                  if (logRecord != null &&
                    ...
                    StringUtils.isNotBlank(logRecord.st) &&
                    ...
                    if (StringUtils.isBlank(logRecord.et)) {
                      logRecord.et = logRecord.st
                    }

                    //把新用户的会话中的所有记录都标记为ifo=true
                    val ifo = resource.hget(linkNewUserRedisKey, logRecord.at.trim)
                    if (StringUtils.isNotBlank(ifo)) {
                      logRecord.ifo = ifo
                    }

                    // val media_id = mediaMap.value.get(logRecord.ag_ald_link_key.trim)
                    val media_id = linkMap.get(logRecord.ag_ald_link_key.trim)
        ...
        println("rddcount:" + logRecordRdd.count())
        ...
        LinkAllTask.allStat_second(baseRedisKey, taskId, dateStr, logRecordRdd, kafkaProducer, redisPrefix)
