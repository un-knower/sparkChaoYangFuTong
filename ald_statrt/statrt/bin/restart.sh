...
usage='
     用法: restart.sh job [repartitionNum]\n
...
if [ -z "$appId" ]; then
    echo "ERROR: The job ${mainClass} does not started!"
    exit 1
fi

echo ${mainClass}" is running ,appId="${appId}
...
su ${submitUser}
echo "Killing job "${mainClass}",appId="${appId}
yarn application -kill ${appId}


echo "Restarting job "${mainClass}
