...
/**
  ...
  */
object AllUserSevenDays {
  def main(args: Array[String]): Unit = {
    /**
      * 对一个时间段的日志数据进行读取
      */
    Logger.getLogger("org").setLevel(Level.WARN) //gcs:调整Log日志的优先级
    ...
    val df=ArgsTool.getTencentSevenOrThirtyDF(args,sparkseesion,"7") //gcs:将7天或者30天的离线数据获取到。df是一个DataFrame类型的数据
    df.createTempView("sum_tables") //gcs:将获取到的数据存储到一个临时的视图当中

    /**
      * -------------------------趋势分析、手机、场景值的数据源------------------------
      */
    val df_sum = df.filter("ev = 'app' and ak != '' and ak != 'null'") //gcs:将 "ev = 'app' and ak != '' and ak != 'null'" 的数据过滤出来放到 df_sum 当中
    val UpDataTime = new Timestamp(System.currentTimeMillis()) //gcs:使用当前的Long类型的时间来创造出一个Timestamp对象
    //    df.filter("")
    //    //    df.show(100)

    df_sum.createTempView("senven_tmp") //gcs:创建一个临时的视图
    //
    //    //连接数据库
//    val url = DBConf.url
...
//    prop.setProperty("password", DBConf.password)
    ...
    val url = ConfigurationUtil.getProperty("jdbc.url") //gcs:获得JDBC的驱动

    val prop = new Properties()

    prop.put("driver", ConfigurationUtil.getProperty("jdbc.driver")) //gcs:设置MySql的JDBC的驱动
    prop.setProperty("user", ConfigurationUtil.getProperty("jdbc.user")) //gcs:设置MySql的JDBS的用户名
    prop.setProperty("password", ConfigurationUtil.getProperty("jdbc.pwd"))
    //新用户数+访问人数+访问次数+打开次数+次均停留时长+跳出率
    //计算手机品牌、手机型号
    val name = sparkseesion.read.jdbc(url, "phone_model", prop) //gcs:调用jdbc函数，从表为 phone_model当中的数据读取出来
    name.createTempView("phonename") //gcs: 创建 phonename视图
    //连接表（包含phone_model中的brand）

    val phone_brand = sparkseesion.sql("SELECT a.ak ak,a.uu uu,a.scene scene,b.name name,b.brand brand FROM senven_tmp a left join phonename b on a.pm = b.uname").distinct() //gcs:进行mysql的查询以及做到去重
    //    phone_brand.show()
    ...
    // 二维码每天的扫码人数
    val grouped_df = qr_info_df.join( //gcs:先进行join的操作。然后再进行select的操作
      page_df,
    ...
    /**
      ...
      */
    //受访页
    df.filter("ev ='page'") //gcs:先把所有的上报类型封装进一个DataFrame当中，之后使用filter函数，将ev=page的类型筛选出来
      .select(
        df("ak").alias("app_key"),
    ...
    /**
      ...
      */
    //访问深度
    sparkseesion.sql("SELECT ak app_key,uu,at, (case when count(pp)=1  then '1' " +
      "when count(pp)=2  then '2' " +
      "when count(pp)=3 then '3' " +
      "when count(pp)=4 then '4' " +
      "when count(pp)>=5 and count(pp)<=10 then '5' " +
      "else '6' end) page_depth FROM sum_tables where ev ='page' GROUP BY ak,uu,at")
      .createTempView("df_depth_pp")


    //gcs:计算打开次数。这是计算用户打开微信小程序的首页的次数
    sparkseesion.sql("select ak app_key ,uu,count(at) as pps,at from sum_tables where ev ='app' group by ak,uu,at")
      .createTempView("df_depth_at")


    //打开次数与标记表连接
    sparkseesion.sql("select b.app_key app_key,b.uu uu,b.page_depth visit_depth,a.pps from df_depth_pp b left join df_depth_at a on b.app_key=a.app_key and b.uu=a.uu and b.at=a.at")
      .na.fill(0)
    ...
    //    //访问人数和访问次数
    //    val a_result = sparkseesion.sql("SELECT app_key,visit_frequency,count(0) as num_of_user, sum(pps) as vists_times from df_frequency_result group by app_key,visit_frequency ")
    //    a_result.createTempView("result_frequency")
    //    a_result.show()
    ...
    //    //访问人数 访问次数
    //    sparkseesion.sql("SELECT app_key,visit_duration,count(0) as num_of_user, " +
    //      "  sum(pps) as vists_times " +
    //      " from duration_result group by app_key,visit_duration  ").show()
    ...
    val arr = Array(
      "app_key",
      "app_key,scene type_value",
      "app_key,scene_group_id type_value",
      "app_key,qr_key type_value",
      "app_key,qr_group_key type_value",
      "qr_key_all","qr_group_key_all",
      "app_key,model type_value",
      "app_key,brand type_value",
      "app_key,province type_value",
      "app_key,city type_value",
      "app_key,entrance type_value",
      "app_key,respondents type_value",
      "app_key,visit_depth type_value",
      "app_key,visit_duration type_value",
      "entrance", "respondents"
    )
    ...
    for (tmp_args <- arr) {
      ...
      // 批量入库
      allusersevendaysForeachPartition(visiter_count_result)

    }
    sparkseesion.stop()
  }

  private def allusersevendaysForeachPartition(userDf:DataFrame): Unit = {

    // 逐行入库
    userDf.foreachPartition((rows:Iterator[Row]) => {
//      val conn = JdbcUtil.getConn()
//      val statement = conn.createStatement
      val sqlText = "insert into ald_seven_user_tmp (app_key,day,type,type_value,visitor_count,update_at) values (?,?,?,?,?,?) ON DUPLICATE KEY UPDATE update_at=?, visitor_count=?"
