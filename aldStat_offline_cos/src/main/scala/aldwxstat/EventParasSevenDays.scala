...
object EventParasSevenDays {
  def main(args: Array[String]): Unit = {
    ...
    // 获取-d参数
    val yday = TimeUtil.processArgs(args)


    //    val rdd_df = Aldstat_args_tool.analyze_args(args,spark,DBConf.hdfsUrl).filter("ev='event'and tp !='ald_error_message' and tp !='ald_reachbottom' and tp !='ald_share_click' and tp !='ald_share_status' and tp !='ald_share_user' and tp !='ald_share_chain' and tp !='ald_pulldownrefresh' and tp !='js_err' and ak!='d6be9d076493c2c3f51b0dbf1ac5cab0'").cache()
    val rdd_df = ArgsTool.getLogs(args, spark, ConfigurationUtil.getProperty("tongji.parquet")).filter("ev='event'and tp !='ald_error_message' and tp !='ald_reachbottom' and tp !='ald_share_click' and tp !='ald_share_status' and tp !='ald_share_user' and tp !='ald_share_chain' and tp !='ald_pulldownrefresh' and tp !='js_err' and ak!='d6be9d076493c2c3f51b0dbf1ac5cab0'").cache()

    var map: mutable.Map[String, String] = scala.collection.mutable.Map()

    val jSONRDD = rdd_df.toJSON.rdd.map(line => {
      map.clear()
    ...
    //将数据和元信息匹配起来
    val df_1 = spark.createDataFrame(rsRDD, schema)
    df_1.createTempView("result")
    ...
    val rs = spark.sql("select ak,tp,udf(ss),ct_key,ct_value,ct_count,now() from result")
    //    rs.filter("ak='6b5dae0b56e5cfe1f79871045e821907'").show(200)

    var statement: Statement = null
    rs.repartition(1).foreachPartition((row: Iterator[Row]) => {
      //连接
      ...
      //          val update_at = r(6)
      //
      //          val sql = "insert into aldstat_7days_event_paras (app_key,day,ev_id,event_key,ev_paras_name,ev_paras_value,ev_paras_count,update_at)" + s"values ('${app_key}','${day}','${ev_id}','${event_key}','${ev_paras_name}','${ev_paras_value}','${ev_paras_count}','${update_at}') ON DUPLICATE KEY UPDATE ev_paras_count='${ev_paras_count}',update_at='${update_at}'"
      ...
      val sqlText = "insert into aldstat_7days_event_paras (app_key,day,ev_id,event_key,ev_paras_name,ev_paras_value,ev_paras_count,update_at)" + s"values (?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE ev_paras_count=?,update_at=?"
      row.foreach(r => {
        val app_key = r.get(0)
        val day = yday
        val ev_id = r.get(1)
        ...
        //          ON DUPLICATE KEY UPDATE ev_paras_count='${ev_paras_count}',update_at='${update_at}'"
