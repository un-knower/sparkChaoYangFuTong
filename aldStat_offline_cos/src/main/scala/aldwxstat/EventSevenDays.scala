...
object EventSevenDays {
  def main(args: Array[String]): Unit = {
    ...
    result.foreachPartition((row: Iterator[Row]) => {
      ...
      //
      //        val sql = "insert into aldstat_7days_event (app_key,day,ev_id,event_key,trigger_user_count,trigger_count,avg_trigger_count,update_at)"+s"values ('${app_key}','${day}','${ev_id}','${event_key}','${trigger_user_count}','${trigger_count}','${avg_trigger_count}','${update_at}') ON DUPLICATE KEY UPDATE trigger_user_count='${trigger_user_count}', trigger_count='${trigger_count}',avg_trigger_count='${avg_trigger_count}',update_at='${update_at}'"
      ...
      val sqlText = "insert into aldstat_7days_event (app_key,day,ev_id,event_key,trigger_user_count,trigger_count,avg_trigger_count,update_at)" + s"values (?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE trigger_user_count=?, trigger_count=?,avg_trigger_count=?,update_at=?"
