...
object EventDaily {
  def main(args: Array[String]): Unit = {
    ...
    //    val datafream = Aldstat_args_tool.analyze_args(args,spark,DBConf.hdfsUrl).filter("ev='event'").cache()
    val datafream = ArgsTool.getLogs(args, spark, ConfigurationUtil.getProperty("tongji.parquet")).filter("ev='event'").cache()
    datafream.createTempView("daily_event")
    ...
    result.foreachPartition((row: Iterator[Row]) => {
      ...
      //
      //        val sql = "insert into aldstat_daily_event (app_key,day,ev_id,event_key,trigger_user_count,trigger_count,avg_trigger_count,update_at)"+s"values ('${app_key}','${day}','${ev_id}','${event_key}','${trigger_user_count}','${trigger_count}','${avg_trigger_count}','${update_at}') ON DUPLICATE KEY UPDATE trigger_user_count='${trigger_user_count}', trigger_count='${trigger_count}',avg_trigger_count='${avg_trigger_count}',update_at='${update_at}'"
      ...
      val sqlText = "insert into aldstat_daily_event (app_key,day,ev_id,event_key,trigger_user_count,trigger_count,avg_trigger_count,update_at)" + s"values (?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE trigger_user_count=?, trigger_count=?,avg_trigger_count=?,update_at=?"
