...
class PositionHourlyDetails(spark: SparkSession) extends LinkAnalysis {
  ...
  def totalStayTime(): Unit = {
    ...
    spark.sql(
      """
        ...
        |(
        |select app_key,hour,ag_ald_position_id,case when w.v in ('7.0.0') then w.dr/1000 else w.dr end as dr  from
        |(
  ...
  def bounceRate(): Unit = {
    spark.sql(
      """
        |select tpc.app_key app_key,tpc.hour hour,tpc.ag_ald_position_id ag_ald_position_id,cast(vpo.one_page_count/tpc.total_page_count as float) bounce_rate
        |from total_page_count tpc
        |left join visit_page_once vpo
        |on tpc.app_key = vpo.app_key and tpc.hour=vpo.hour and tpc.ag_ald_position_id=vpo.ag_ald_position_id
  ...
  def insert2db(): Unit = {
    ...
    val positionDailyDetail = spark.sql(
      """
        |select vc.app_key,vc.hour,vc.ag_ald_position_id,vc.visitor_count,oc.open_count,tpc.total_page_count,nu.new_comer_count,
    ...
    positionDailyDetail.na.fill("0").na.fill(0).foreachPartition(rows => {
      ...
      val sqlText =
        """
          |insert into aldstat_hourly_position(app_key,day,hour,position_id,position_visitor_count,position_open_count,position_page_count,
      ...
      rows.foreach(row => {
        ...
        params.+=(Array[Any](app_key, day, hour, ag_ald_position_id, visitor_count, open_count, total_page_count,
