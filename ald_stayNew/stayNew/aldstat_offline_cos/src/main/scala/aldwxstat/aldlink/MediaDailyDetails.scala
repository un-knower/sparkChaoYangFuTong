...
class MediaDailyDetails(spark: SparkSession) extends LinkAnalysis {
  ...
  def totalStayTime(): Unit = {
    spark.sql(
      """
        ...
        |(
        |select app_key,ag_ald_media_id,case when w.v in ('7.0.0') then w.dr/1000 else w.dr end as dr  from
        |(
  ...
  def bounceRate(): Unit = {
    spark.sql(
      """
        |select tpc.app_key app_key,tpc.ag_ald_media_id ag_ald_media_id,cast(vpo.one_page_count/tpc.total_page_count as float) bounce_rate
        |from total_page_count tpc
        |left join visit_page_once vpo
        |on tpc.app_key = vpo.app_key and tpc.ag_ald_media_id=vpo.ag_ald_media_id
  ...
  def insert2db(): Unit = {
    ...
    val mediaDailyDetail = spark.sql(MessageFormat.format(
      """
        |select vc.app_key,vc.ag_ald_media_id,vc.visitor_count,oc.open_count,tpc.total_page_count,nu.new_comer_count,
    ...
    mediaDailyDetail.na.fill("0").na.fill(0).foreachPartition(rows => {
      ...
      val sqlText =
        """
          |insert into aldstat_daily_media(app_key,day,media_id,media_visitor_count,media_open_count,media_page_count,
      ...
      rows.foreach(row => {
        ...
        params.+=(Array[Any](app_key, day, ag_ald_media_id, visitor_count, open_count, total_page_count,
