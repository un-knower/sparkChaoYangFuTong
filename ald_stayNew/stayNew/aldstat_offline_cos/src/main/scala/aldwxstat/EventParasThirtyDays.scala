...
object EventParasThirtyDays {
  def main(args: Array[String]): Unit = {
    ...
    rs.foreachPartition((row: Iterator[Row]) => {
      ...
      //          val update_at = r(6)
      //
      //          val sql = "insert into aldstat_30days_event_paras (app_key,day,ev_id,event_key,ev_paras_name,ev_paras_value,ev_paras_count,update_at)" + s"values ('${app_key}','${day}','${ev_id}','${event_key}','${ev_paras_name}','${ev_paras_value}','${ev_paras_count}','${update_at}') ON DUPLICATE KEY UPDATE ev_paras_count='${ev_paras_count}',update_at='${update_at}'"
      ...
      val sqlText = "insert into aldstat_30days_event_paras (app_key,day,ev_id,event_key,ev_paras_name,ev_paras_value,ev_paras_count,update_at)" + s"values (?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE ev_paras_count=?,update_at=?"
