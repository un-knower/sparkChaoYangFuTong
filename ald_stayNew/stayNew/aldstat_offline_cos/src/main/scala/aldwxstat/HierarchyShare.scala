...
object HierarchyShare {
  def main(args: Array[String]): Unit = {
    ...
    val rdd = df_tmp.toJSON.rdd.map(line => {
      ...
      Row(uu,  ak, src1, src2,tp, ct, ifo, ev,  at)
    })
    ...
    //一度分享带来新增
    val share_new_first = sparkseesion.sql("SELECT ak,COUNT(DISTINCT uu) new_count FROM share WHERE ev='event'  AND tp='ald_share_click' and (src1='null' or src1!='') and uu in (select uu from share where ev='app' and ifo='true') GROUP BY ak")
    share_new_first.createTempView("share_new_first")

    //一度分享打开数（回流量）
    val share_open_first = sparkseesion.sql("SELECT ak ,COUNT(at) share_open_count FROM share WHERE ev='event' and tp='ald_share_click' and (src1='null' or src1!='') GROUP BY ak")
    ...
    //二度分享带来新增
    val share_new_second = sparkseesion.sql("SELECT ak,COUNT(DISTINCT uu) new_count FROM share WHERE ev='event'  AND tp='ald_share_click' and src2!='' and uu in (select uu from share where ev='app' and ifo='true' and src2!='') GROUP BY ak")
    share_new_second.createTempView("share_new_second")

    //二度分享打开数（回流量）
    val share_open_second = sparkseesion.sql("SELECT ak ,COUNT(at) share_open_count FROM share WHERE ev='event' and tp='ald_share_click' and src2!='' GROUP BY ak")
    ...
    sparkseesion.sqlContext.cacheTable("result")
