...
object AldPhoneBrandDetails {
  ...
  def fillDailyData(args: Array[String], spark: SparkSession): Unit = {
    ...
    val logs = ArgsTool.getDailyDataFrame(spark, ConfigurationUtil.getProperty("tongji.parquet")).filter("pm !='undefined'")
    println("创建原始日志视图...")
    logs.createOrReplaceTempView("logs")

    spark.sql(s"SELECT a.ak,a.uu,a.dr,a.ifo,a.ev,a.at,a.pp,a.st,b.brand ${dimensionAnalysis} FROM logs a left join phone_model b on a.pm = b.uname")
      .createOrReplaceTempView("phone_model_data_all")
    //数据源分三种：app（小程序）、page（页面）、event（时间），只有分享取ev=event
    //    获取需要在ev=app中计算的指标的字段
    spark.sql(s"select ak,uu,at,${dimensionAnalysis},ifo,dr from phone_model_data_all where ev='app'")
      .createOrReplaceTempView("app_view")
    spark.sqlContext.cacheTable("app_view")
    //    获取需要在ev=page中计算的指标的字段
    spark.sql(s"select ak,uu,${dimensionAnalysis},pp,at,dr from phone_model_data_all where ev='page'")
      .createOrReplaceTempView("page_view")
    spark.sqlContext.cacheTable("page_view")

    //    创建对象，将sparksession和需要执行的维度出进去province
    val brandDailyDetails: PublicModularDetails = new PublicModularDetails(spark, dimensionAnalysis)
    //    调用最终执行的接口函数
    adDailyData(brandDailyDetails)
    //    数据入库
    insert2db(spark)
    //    清除缓存表
  ...
  /**
    ...
    * @param brandAnalysis
    */
  def adDailyData(brandAnalysis: PublicModularDetails): Unit = {
    brandAnalysis.newUser()
    brandAnalysis.visitorCount()
    brandAnalysis.openCount()
    brandAnalysis.totalPageCount()
    brandAnalysis.totalStayTime()
    brandAnalysis.secondaryAvgStayTime()
    brandAnalysis.visitPageOnce()
    brandAnalysis.bounceRate()
  }
  ...
  def insert2db(spark: SparkSession): Unit = {
    ...
    result.foreachPartition((rows: Iterator[Row]) => {
      ...
      val sqlText = s"insert into aldstat_daily_phonebrand (app_key,day,new_user_count, visitor_count,open_count,page_count,secondary_stay_time,total_stay_time,bounce_rate,one_page_count,update_at,brand)" +
        s"values (?,?,?,?,?,?,?,?,?,?,?,?) ON " +
      ...
      rows.foreach(r => {
        ...
        println(s"total_stay_time 是 $total_stay_time")
