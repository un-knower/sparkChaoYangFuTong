...
class PublicMysqlInsertDetails(spark: SparkSession) extends PublicMysqlInsertData {
  ...
  def trendInsert2db(): Unit = {
    ...
    result.foreachPartition((rows: Iterator[Row]) => {
      val params = new ArrayBuffer[Array[Any]]() //数组，存储sql模板需要的数据
      val sqlText = s"insert into aldstat_trend_analysis (app_key,day,new_comer_count, visitor_count,open_count,total_page_count,secondary_avg_stay_time,total_stay_time,daily_share_count,bounce_rate,update_at,one_page_count)" +
        s"values (?,?,?,?,?,?,?,?,?,?,?,?) ON " +
        s"DUPLICATE KEY UPDATE new_comer_count=?, visitor_count=?, open_count=?,total_page_count=?, secondary_avg_stay_time=?,total_stay_time=?, daily_share_count=?, bounce_rate=?,update_at=?,one_page_count=?"
      ...
      rows.foreach(r => {
        ...
        val secondary_avg_stay_time = r.get(5)
        val new_comer_count = r.get(6)
        val one_page_count = r.get(7)
        val bounce_rate = r.get(8)
        val daily_share_count = r.get(9)

        params.+=(Array[Any](app_key, day, new_comer_count, visitor_count, open_count, total_page_count, secondary_avg_stay_time, total_stay_time, daily_share_count, bounce_rate, update_at, one_page_count, new_comer_count, visitor_count, open_count, total_page_count, secondary_avg_stay_time, total_stay_time, daily_share_count, bounce_rate, update_at, one_page_count))
  ...
  def hourTrendInsert2db(): Unit = {
    ...
    result.foreachPartition((rows: Iterator[Row]) => {
      val params = new ArrayBuffer[Array[Any]]() //数组，存储sql模板需要的数据
      val sqlText = s"insert into aldstat_hourly_trend_analysis (app_key,day,new_comer_count, visitor_count,open_count,total_page_count,secondary_avg_stay_time,total_stay_time,daily_share_count,bounce_rate,update_at,hour,one_page_count)" +
        s"values (?,?,?,?,?,?,?,?,?,?,?,?,?) ON " +
        s"DUPLICATE KEY UPDATE new_comer_count=?, visitor_count=?, open_count=?,total_page_count=?, secondary_avg_stay_time=?, total_stay_time=?, daily_share_count=?, bounce_rate=?,update_at=?,one_page_count=?"
      ...
      rows.foreach(r => {
        ...
        params.+=(Array[Any](app_key, day, new_comer_count, visitor_count, open_count, total_page_count, secondary_avg_stay_time, total_stay_time, daily_share_count, bounce_rate, update_at, hour, one_page_count, new_comer_count, visitor_count, open_count, total_page_count, secondary_avg_stay_time, total_stay_time, daily_share_count, bounce_rate, update_at, one_page_count))
      })
      JdbcUtil.doBatch(sqlText, params) //批量入库
    })
  }
