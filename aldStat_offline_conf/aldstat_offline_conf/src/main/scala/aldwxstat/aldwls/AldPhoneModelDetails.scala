...
object AldPhoneModelDetails {
  ...
  def fillDailyData(args: Array[String], spark: SparkSession): Unit = {
    ...
    val logs = ArgsTool.getLogs(args, spark, ConfigurationUtil.getProperty("tongji.parquet")).filter("pm !='undefined'")
    println("创建原始日志视图...")
    ...
    spark.sql(s"select ak,uu,at,${dimensionAnalysis},ifo,dr from phone_model_data_all where ev='app'").createOrReplaceTempView("app_view")
