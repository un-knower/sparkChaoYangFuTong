...
object AldProvinceDetails {
  ...
  def fillDailyData(args: Array[String], spark: SparkSession): Unit = {
    val logs = ArgsTool.getLogs(args, spark, ConfigurationUtil.getProperty("tongji.parquet")).filter("province != 'null'")
    ...
    spark.sql("select ak,uu,at,province,ifo,dr from logs where ev='app'").createOrReplaceTempView("app_view")
    spark.sqlContext.cacheTable("app_view")
    //    获取需要在ev=page中计算的指标的字段
    spark.sql("select ak,uu,province,pp,at,dr from logs where ev='page'")
