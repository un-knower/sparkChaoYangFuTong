...
/**
  ...
  */
class MediaDailyDetails(spark: SparkSession) extends LinkAnalysis {

  //点击量(click_count)
  def clickCount(): Unit = {

    spark.sql("SELECT ak app_key,ag_ald_media_id,COUNT(pp) click_count FROM visitora_page GROUP BY ak,ag_ald_media_id")
  ...
  def newUser(): Unit = {
    ...
    spark.sql("SELECT ak,ag_ald_media_id,uu FROM visitora_page GROUP BY ak,ag_ald_media_id,uu")
      .createOrReplaceTempView("nu_page_daily")
    spark.sql(
      """select ak app_key,ag_ald_media_id,count(uu) new_comer_count
        |from
        |(
        |select page.ak ak,page.ag_ald_media_id ag_ald_media_id,page.uu
        |from nu_app_daily app
        |inner join nu_page_daily page
        |on page.ak=app.ak and page.uu=app.uu
        |)
        |group by ak,ag_ald_media_id
  ...
  //授权用户(authuser_count)
  def atSession2(): Unit = {
    spark.sql("SELECT  ak, ag_ald_media_id, at  FROM logs where ak!='null'and at!='null' and ag_ald_media_id!='null'and ifo='true' and (scene!='1020'or scene!='1089' or scene!='1001' or scene!='1023' or scene!='1069' or scene!='1024' or scene!='1006') GROUP BY ak,ag_ald_media_id,at")
      .createOrReplaceTempView("at_session2")
  }


  def authuserCount(): Unit = {
    spark.sql("SELECT a.ak app_key,a.ag_ald_media_id, count(DISTINCT b.img) authuser_count from at_session2 a left join (select ak ,at,img from logs where img!='' and img!='null') b on a.ak=b.ak and a.at=b.at GROUP BY a.ak,a.ag_ald_media_id")
  ...
  //访问次数，页面总访问量
  def atSession(): Unit = {
    spark.sql("SELECT  ak, ag_ald_media_id, at  FROM logs where ev='page' and ak!='null'and at!='null' and ag_ald_media_id!='null'and(scene!='1020'or scene!='1089'  or scene!='1001' or scene!='1023' or scene!='1069' or scene!='1024' or scene!='1006') GROUP BY ak,ag_ald_media_id,at")
      .createOrReplaceTempView("at_session")
  }


  def totalPageCount(): Unit = {
    spark.sql("SELECT a.ak app_key,a.ag_ald_media_id, count(b.pp) total_page_count from at_session a left join (select ak ,at,pp from logs where ev='page') b on a.ak=b.ak and a.at=b.at GROUP BY a.ak,a.ag_ald_media_id")
  ...
  /*def totalPageCount(): Unit = {
    spark.sql("SELECT ak app_key,ag_ald_media_id,COUNT(pp) total_page_count FROM visitora_page GROUP BY ak,ag_ald_media_id")
      .createOrReplaceTempView("total_page_count")
  ...
  def totalStayTime(): Unit = {
    spark.sql(
      """
        |select app_key,ag_ald_media_id,sum(dr) total_stay_time from
        |(
        |select app_key,ag_ald_media_id,case when w.v >='7.0.0' then w.dr/1000 else w.dr end as dr  from
        |(
        |select page.ak app_key,page.ag_ald_media_id ag_ald_media_id,max(page.dr) dr,page.v
        |from visitora_page page
        |left join app_tmp app
        |on page.ak=app.ak and page.at=app.at
        |group by app_key,ag_ald_media_id,page.v,page.at
        |) w
        |)
  ...
  def visitPageOnce(): Unit = {
    spark.sql(//|ak，at，所有page的访问次数=1|
      """
        |select tmp.ak app_key,tmp.ag_ald_media_id ag_ald_media_id,sum(tmp.cp) once_page
        |from (SELECT ak,ag_ald_media_id,at,COUNT(pp) cp FROM visitora_page GROUP BY ak,ag_ald_media_id,at,pp) tmp
        |group by tmp.ak,tmp.ag_ald_media_id,tmp.at
      """.stripMargin).createOrReplaceTempView("visit_page_sum_once")
    spark.sql(//|ak，at，所有page的访问次数=1|
      """
        |select app_key,ag_ald_media_id,sum(once_page) one_page_count
  ...
  def bounceRate(): Unit = {
    spark.sql(
      """
        |select oc.app_key app_key,oc.ag_ald_media_id ag_ald_media_id,cast(vpo.one_page_count/oc.open_count as float) bounce_rate
        |from open_count oc
        |left join visit_page_once vpo
        |on oc.app_key = vpo.app_key and oc.ag_ald_media_id=vpo.ag_ald_media_id
  ...
  def insert2db(): Unit = {
    ...
    val mediaDailyDetail = spark.sql(MessageFormat.format(
      """
        |select vc.app_key,vc.ag_ald_media_id,vc.visitor_count,ac.authuser_count,oc.open_count,tpc.total_page_count,cc.click_count,nu.new_comer_count,
        ...
        |on vc.app_key=ac.app_key and vc.ag_ald_media_id=ac.ag_ald_media_id
        |left join open_count oc
        |on vc.app_key=oc.app_key and vc.ag_ald_media_id=oc.ag_ald_media_id
        |left join total_page_count tpc
        |on vc.app_key=tpc.app_key and vc.ag_ald_media_id=tpc.ag_ald_media_id
        |left join click_count cc
        |on vc.app_key=cc.app_key and vc.ag_ald_media_id=cc.ag_ald_media_id
        |left join new_user_daily nu
        |on vc.app_key=nu.app_key and vc.ag_ald_media_id=nu.ag_ald_media_id
        |left join total_stay_time tst
        ...
        |left join visit_page_once vpo
        |on vc.app_key=vpo.app_key and vc.ag_ald_media_id=vpo.ag_ald_media_id
        |left join bounce_rate bounce
        |on vc.app_key=bounce.app_key and vc.ag_ald_media_id=bounce.ag_ald_media_id
      """.stripMargin, day, new Timestamp(System.currentTimeMillis()))) //.show()

    mediaDailyDetail.na.fill("0").na.fill(0).foreachPartition(rows => {
      val params = new ArrayBuffer[Array[Any]]()
      val sqlText =
        """
          |insert into aldstat_daily_media(app_key,day,media_id,media_visitor_count,media_authuser_count,media_open_count,media_page_count,media_click_count,
          |media_newer_for_app,total_stay_time,secondary_stay_time,one_page_count,bounce_rate,update_at)
          |values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)
          |on duplicate key update
          |media_visitor_count=?,media_authuser_count=?,media_open_count=?,media_page_count=?,media_click_count=?,media_newer_for_app=?,total_stay_time=?,
      ...
      rows.foreach(row => {
        ...
        params.+=(Array[Any](app_key, day, ag_ald_media_id, visitor_count,authuser_count, open_count, total_page_count,click_count,
