...
class PositionDailyDeails(spark: SparkSession) extends LinkAnalysis {
  ...
  def clickCount(): Unit = {
    ...
    spark.sql("SELECT ak app_key,ag_ald_position_id,COUNT(pp) click_count FROM visitora_page GROUP BY ak,ag_ald_position_id")
  ...
  //授权用户(authuser_count)
  def atSession2(): Unit = {
    spark.sql("SELECT  ak, scene, at  FROM logs where ak!='null'and at!='null' and ag_ald_position_id!='null'and ifo='true' and (scene!='1020'or scene!='1089'  or scene!='1001' or scene!='1023' or scene!='1069' or scene!='1024' or scene!='1006') GROUP BY ak,scene,at")
      .createOrReplaceTempView("at_session2")
  }


  def authuserCount(): Unit = {
    spark.sql("SELECT a.ak app_key,case when a.scene in (1058,1035,1014,1038) then a.scene else '其它' end as ag_ald_position_id, count(DISTINCT b.img) authuser_count from at_session2 a left join (select ak ,at,img from logs where img!='' and img!='null') b on a.ak=b.ak and a.at=b.at GROUP BY a.ak,ag_ald_position_id")
  ...
  def atSession(): Unit = {
    spark.sql("SELECT  ak,scene, at  FROM logs where ev='page' and ak!='null'and at!='null' and ag_ald_position_id!='null' and scene!='null'and(scene!='1020'or scene!='1089'  or scene!='1001' or scene!='1023' or scene!='1069' or scene!='1024' or scene!='1006') GROUP BY ak,scene,at")
      .createOrReplaceTempView("at_session")
  }


  def totalPageCount(): Unit = {
    spark.sql("SELECT a.ak app_key,case when a.scene in (1058,1035,1014,1038) then a.scene else '其它' end as ag_ald_position_id, count(b.pp) total_page_count from at_session a left join (select ak ,at,pp from logs where ev='page') b on a.ak=b.ak and a.at=b.at GROUP BY a.ak,ag_ald_position_id")
  ...
  def totalStayTime(): Unit = {
    ...
    spark.sql(
      """
        ...
        |(
        |select app_key,ag_ald_position_id,case when w.v >='7.0.0' then w.dr/1000 else w.dr end as dr  from
        |(
  ...
  def bounceRate(): Unit = {
    spark.sql(
      """
        |select oc.app_key app_key,oc.ag_ald_position_id ag_ald_position_id,cast(vpo.one_page_count/oc.open_count as float) bounce_rate
        |from open_count oc
        |left join visit_page_once vpo
        |on oc.app_key = vpo.app_key and oc.ag_ald_position_id=vpo.ag_ald_position_id
  ...
  def insert2db(): Unit = {
    ...
    val positionDailyDetail = spark.sql(MessageFormat.format(
      """
        |select vc.app_key,vc.ag_ald_position_id,vc.visitor_count,ac.authuser_count,oc.open_count,tpc.total_page_count,cc.click_count,nu.new_comer_count,
        ...
        |on vc.app_key=ac.app_key and vc.ag_ald_position_id=ac.ag_ald_position_id
        ...
        |on vc.app_key=cc.app_key and vc.ag_ald_position_id=cc.ag_ald_position_id
    ...
    positionDailyDetail.na.fill("0").na.fill(0).foreachPartition(rows => {
      ...
      val sqlText =
        """
          |insert into aldstat_daily_position(app_key,day,position_id,position_visitor_count,position_authuser_count,position_open_count,position_page_count,position_click_count,
      ...
      rows.foreach(row => {
        ...
        params.+=(Array[Any](app_key, day, ag_ald_position_id, visitor_count,authuser_count, open_count, total_page_count,click_count,
