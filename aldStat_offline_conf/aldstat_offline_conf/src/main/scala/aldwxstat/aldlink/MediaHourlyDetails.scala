...
class MediaHourlyDetails(spark: SparkSession) extends LinkAnalysis {
  ...
  def clickCount(): Unit = {
    ...
    spark.sql("SELECT ak app_key,hour,ag_ald_media_id,COUNT(pp) click_count FROM visitora_page GROUP BY ak,hour,ag_ald_media_id")
  ...
  //授权用户(authuser_count)
  def atSession2(): Unit = {
    spark.sql("SELECT  ak,hour, ag_ald_media_id, at  FROM logs where ak!='null'and at!='null' and ag_ald_media_id!='null'and ifo='true' and (scene!='1020'or scene!='1089' or scene!='1001' or scene!='1023' or scene!='1069' or scene!='1024' or scene!='1006') GROUP BY ak,hour,ag_ald_media_id,at")
      .createOrReplaceTempView("at_session2")
  }


  def authuserCount(): Unit = {
    spark.sql("SELECT a.ak app_key,a.hour,a.ag_ald_media_id, count(DISTINCT b.img) authuser_count from at_session2 a left join (select ak ,at,img from logs where img!='' and img!='null') b on a.ak=b.ak and a.at=b.at GROUP BY a.ak,a.hour,a.ag_ald_media_id")
  ...
  def atSession(): Unit = {
    spark.sql("SELECT  ak,hour, ag_ald_media_id, at  FROM logs where ev='page' and ak!='null'and at!='null' and ag_ald_media_id!='null'and(scene!='1020'or scene!='1089' or scene!='1001' or scene!='1023' or scene!='1069' or scene!='1024' or scene!='1006') GROUP BY ak,hour,ag_ald_media_id,at")
      .createOrReplaceTempView("at_session")
  }


  def totalPageCount(): Unit = {
    spark.sql("SELECT a.ak app_key,a.hour,a.ag_ald_media_id,count(b.pp) total_page_count from at_session a left join (select ak ,at,pp from logs where ev='page') b on a.ak=b.ak and a.at=b.at GROUP BY a.ak,a.hour,a.ag_ald_media_id")
...
*/
  ...
  def totalStayTime(): Unit = {
    ...
    spark.sql(
      """
        ...
        |(
        |select app_key,hour,ag_ald_media_id,
        |case when w.v >='7.0.0' then w.dr/1000 else w.dr end as dr from
        |(
        |select page.ak app_key,page.hour hour,page.ag_ald_media_id ag_ald_media_id,max(page.dr) dr,page.v
        |from visitora_page page
        |left join app_tmp app
        |on page.ak=app.ak and page.at=app.at and page.hour=app.hour
        |group by page.ak,page.hour,ag_ald_media_id,page.v,page.at
        |) w
        |)
  ...
  def bounceRate(): Unit = {
    spark.sql(
      """
        |select oc.app_key app_key,oc.hour hour,oc.ag_ald_media_id ag_ald_media_id,cast(vpo.one_page_count/oc.open_count as float) bounce_rate
        |from open_count oc
        |left join visit_page_once vpo
        |on oc.app_key = vpo.app_key and oc.hour=vpo.hour and oc.ag_ald_media_id=vpo.ag_ald_media_id
  ...
  def insert2db(): Unit = {
    ...
    val mediaDailyDetail = spark.sql(
      """
        |select vc.app_key,vc.hour,vc.ag_ald_media_id,vc.visitor_count,ac.authuser_count,oc.open_count,tpc.total_page_count,cc.click_count,nu.new_comer_count,
        ...
        |on vc.app_key=ac.app_key and vc.hour=ac.hour and vc.ag_ald_media_id=ac.ag_ald_media_id
        ...
        |on vc.app_key=cc.app_key and vc.hour=cc.hour and vc.ag_ald_media_id=cc.ag_ald_media_id
    ...
    mediaDailyDetail.na.fill("0").na.fill(0).foreachPartition(rows => {
      ...
      val sqlText =
        """
          |insert into aldstat_hourly_media(app_key,day,hour,media_id,media_visitor_count,media_authuser_count,media_open_count,media_page_count,media_click_count,
          |media_newer_for_app,total_stay_time,secondary_stay_time,one_page_count,bounce_rate,update_at)
          |values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
      ...
      rows.foreach(row => {
        ...
        params.+=(Array[Any](app_key, day, hour, ag_ald_media_id, visitor_count,authuser_count, open_count, total_page_count,click_count,
          new_user_daily, total_stay_time, secondary_avg_stay_time, visit_page_once, bounce_rate, update_at,
