...
class LinkHourlyDetails(spark: SparkSession) extends LinkAnalysis {
  ...
  def clickCount(): Unit = {
    ...
    spark.sql("SELECT ak app_key,hour,ag_ald_link_key,COUNT(pp) click_count FROM visitora_page GROUP BY ak,hour,ag_ald_link_key")
  ...
  //授权用户(authuser_count)
  def atSession2(): Unit = {
    spark.sql("SELECT  ak,hour, ag_ald_link_key, at  FROM logs where ak!='null'and at!='null' and ag_ald_link_key!='null'and ifo='true' and (scene!='1020'or scene!='1089'  or scene!='1001' or scene!='1023' or scene!='1069' or scene!='1024' or scene!='1006') GROUP BY ak,hour,ag_ald_link_key,at")
      .createOrReplaceTempView("at_session2")
  }


  def authuserCount(): Unit = {
    spark.sql("SELECT a.ak app_key,a.hour,a.ag_ald_link_key, count(DISTINCT b.img) authuser_count from at_session2 a left join (select ak ,at,img from logs where img!='' and img!='null') b on a.ak=b.ak and a.at=b.at GROUP BY a.ak,a.hour,a.ag_ald_link_key")
  ...
  def atSession(): Unit = {
    println("计算会话")
    spark.sql("SELECT  ak,hour,ag_ald_link_key, at  FROM logs where ev='page' and ak!='null'and at!='null' and ag_ald_link_key!='null'and(scene!='1020'or scene!='1089'  or scene!='1001' or scene!='1023' or scene!='1069' or scene!='1024' or scene!='1006') GROUP BY ak,hour,ag_ald_link_key,at")
  ...
  def totalPageCount(): Unit = {
    println("计算访问次数")
    spark.sql("SELECT a.ak app_key,a.hour,a.ag_ald_link_key,count(b.pp) total_page_count from at_session a left join (select ak ,at,pp from logs where ev='page') b on a.ak=b.ak and a.at=b.at GROUP BY a.ak,a.hour,a.ag_ald_link_key")
  ...
  def totalStayTime(): Unit = {
    ...
    spark.sql(
      """
        ...
        |(
        |select app_key,hour,ag_ald_link_key,
        |case when w.v >='7.0.0' then w.dr/1000 else w.dr end as dr from
        |(
        |select page.ak app_key,page.hour hour,page.ag_ald_link_key ag_ald_link_key,max(page.dr) dr,page.v
        |from visitora_page page
        |left join app_tmp app
        |on page.ak=app.ak and page.at=app.at and page.hour=app.hour
        |group by app_key,page.hour,ag_ald_link_key,page.v,page.at
        |) w
        |)
  ...
  def bounceRate(): Unit = {
    spark.sql(
      """
        |select oc.app_key app_key,oc.hour hour,oc.ag_ald_link_key ag_ald_link_key,cast(vpo.one_page_count/oc.open_count as float) bounce_rate
        |from open_count oc
        |left join visit_page_once vpo
        |on oc.app_key = vpo.app_key and oc.hour=vpo.hour and oc.ag_ald_link_key=vpo.ag_ald_link_key
  ...
  def insert2db(): Unit = {
    ...
    val linkDailyDetail = spark.sql(
      """
        |select vc.app_key,vc.hour,vc.ag_ald_link_key,vc.visitor_count,ac.authuser_count,oc.open_count,tpc.total_page_count,cc.click_count,nu.new_comer_count,
        ...
        |on vc.app_key=ac.app_key and vc.hour=ac.hour and vc.ag_ald_link_key=ac.ag_ald_link_key
        ...
        |on vc.app_key=cc.app_key and vc.hour=cc.hour and vc.ag_ald_link_key=cc.ag_ald_link_key
    ...
    linkDailyDetail.na.fill("0").na.fill(0).foreachPartition(rows => {
      ...
      val sqlText =
        """
          |insert into aldstat_hourly_link(app_key,day,hour,link_key,link_visitor_count,link_authuser_count,link_open_count,link_page_count,link_click_count,
          |link_newer_for_app,total_stay_time,secondary_stay_time,one_page_count,bounce_rate,update_at)
          |values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
          |on duplicate key update link_visitor_count=?,link_authuser_count=?,link_open_count=?,link_page_count=?,link_click_count=?,link_newer_for_app=?,total_stay_time=?,
      ...
      rows.foreach(row => {
        ...
        params.+=(Array[Any](app_key, day, hour, ag_ald_link_key, visitor_count,authuser_count, open_count, total_page_count,click_count,
