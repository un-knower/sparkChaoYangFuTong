...
object EntrancePageSummary {
  def main(args: Array[String]): Unit = {
    ...
    //总的停留时长
    sparkSession.sql("SELECT ak,sum(time) total_time FROM (SELECT ak,pp,cast(max(dr) as int) time FROM jumpDemo GROUP BY ak,pp,at) group by ak").createTempView("time")
