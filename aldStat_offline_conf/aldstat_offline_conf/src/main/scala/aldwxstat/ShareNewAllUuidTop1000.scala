...
object ShareNewAllUuidTop1000 {
  def main(args: Array[String]): Unit = {
    ...
    val rdd = df_tmp.toJSON.rdd.map(line => {
      ...
      //share_path为分享页
      var share_path=""
      val v=jsonLine.get("v")
      try{
        if(JudgeVersion.version(v)&&jsonLine.get("ct_path")!=null&&jsonLine.get("ct_path").toString!=""&&jsonLine.get("ct_path").toString.charAt(0)!='?'){//版本为7以上
          share_path=jsonLine.get("ct_path").toString.split("[?]")(0)
          //替换开头的反斜杠
          if(share_path.charAt(0).toString=="/"){
            share_path=share_path.replaceFirst("[/]","")
          }
        }else {//版本为7以下
          if(jsonLine.get("path")!=null&&jsonLine.get("path")!=""){
            share_path=path.toString
            //替换开头的反斜杠
            if(share_path.charAt(0).toString=="/"){
              share_path=share_path.replaceFirst("[/]","")
            }
      ...
      Row(uu, path, ak, src1, tp, ct, ifo, ev, hour, at,share_path, scene)
    })
    val schema = StructType(List(
      ...
      StructField("share_path", StringType, true),
    ...
    /**
      ...
      */
    //分享次数和人数
    val share_hour = sparkseesion.sql("SELECT ak ,hour,COUNT(distinct uu) share_user_count,COUNT(at) share_count FROM share WHERE ev='event' and ct!='fail' and  tp='ald_share_status' and share_path!='' GROUP BY ak,hour")
    ...
    //点击事件
    val share_click_hour=sparkseesion.sql("select ak,hour,uu from share WHERE ev='event' AND tp='ald_share_click' and path!='null' and (scene='1007' or scene='1008' or scene='1044' or scene='1036')")
    share_click_hour.createTempView("share_click_hour")

    //分享新增
    val share_new_hour=sparkseesion.sql("select b.ak,b.hour,COUNT(DISTINCT b.uu) new_count from share_click_hour a inner join app_new_hour b on a.ak=b.ak and a.uu=b.uu GROUP BY b.ak,b.hour")
    share_new_hour.createTempView("share_new_hour")

    //分享打开数（回流量）
    val share_open_hour = sparkseesion.sql("SELECT ak ,hour,COUNT(at) share_open_count,count(DISTINCT uu) share_open_user_count FROM share WHERE ev='event' and tp='ald_share_click' and path!='null' and (scene='1007' or scene='1008' or scene='1044' or scene='1036') GROUP BY ak,hour")
    ...
    //分享次数和人数
    val share_day = sparkseesion.sql("SELECT ak ,COUNT(distinct uu) share_user_count,COUNT(at) share_count FROM share WHERE ev='event' and ct !='fail' and tp='ald_share_status' and share_path!='' GROUP BY ak")
    share_day.createTempView("share_day")

    //分享带来新增
    val share_new_day = sparkseesion.sql("SELECT ak,COUNT(DISTINCT uu) new_count FROM share WHERE ev='event' and path!='null' and (scene='1007' or scene='1008' or scene='1044' or scene='1036') AND tp='ald_share_click' and uu in (select uu from share where ev='app' and ifo='true' and src1!='') GROUP BY ak")
    share_new_day.createTempView("share_new_day")

    //分享打开数（回流量）
    val share_open_day = sparkseesion.sql("SELECT ak ,COUNT(at) share_open_count,count(DISTINCT uu) share_open_user_count FROM share WHERE ev='event' and tp='ald_share_click' and path!='null' and (scene='1007' or scene='1008' or scene='1044' or scene='1036') GROUP BY ak")
    ...
    //分享次数和人数
    val page_share = sparkseesion.sql("SELECT ak app_key,share_path page_uri,COUNT(share_path) share_count,COUNT(DISTINCT uu) share_user_count FROM share WHERE ev='event' and tp='ald_share_status' and ct!='fail' and share_path!='' GROUP BY ak,share_path")
    page_share.createTempView("page_share")

    //分享带来的新增人数
    val page_click = sparkseesion.sql("SELECT ak app_key,COUNT(DISTINCT uu) new_count,path page_uri FROM share WHERE ev='event' and  tp='ald_share_click' and path!='null' and path!='' and (scene='1007' or scene='1008' or scene='1044' or scene='1036') and uu in (select uu from share where ev='app' and ifo='true') GROUP BY ak,path")
    page_click.createTempView("page_click")

    //分享打开次数/分享打开人数
    val page_share_open_and_user_day = sparkseesion.sql("SELECT ak app_key,path page_uri,COUNT(path) share_open_count,count(DISTINCT uu) share_open_user_count FROM share WHERE ev='event' and tp='ald_share_click' and path!='null' and path!='' and (scene='1007' or scene='1008' or scene='1044' or scene='1036') GROUP BY ak,path")
    ...
    //分享次数
    val user_share = sparkseesion.sql("SELECT ak app_key,uu sharer_uuid,COUNT(at) share_count FROM share WHERE ev='event' and tp='ald_share_status' and ct!='fail' and share_path!='' GROUP BY ak,uu")
    user_share.createTempView("user_share")

    //统计出分享带来的新增人数
    val user_share_click = sparkseesion.sql("SELECT ak app_key,src1 sharer_uuid,COUNT(DISTINCT uu) new_count FROM share WHERE ev='event'  AND tp='ald_share_click' and path!='null' and (scene='1007' or scene='1008' or scene='1044' or scene='1036') and uu in (select uu from share where ev='app' and ifo='true' and src1 is not null) GROUP BY ak,src1").filter("sharer_uuid != ''")
    user_share_click.createTempView("user_share_click")

    //分享打开次数/分享打开人数
    val user_share_open_and_user_day = sparkseesion.sql("SELECT ak app_key,src1 sharer_uuid,COUNT(at) share_open_count,count(DISTINCT uu) share_open_user_count FROM share WHERE ev='event' and tp='ald_share_click' and path!='null' and (scene='1007' or scene='1008' or scene='1044' or scene='1036') GROUP BY ak,src1")
