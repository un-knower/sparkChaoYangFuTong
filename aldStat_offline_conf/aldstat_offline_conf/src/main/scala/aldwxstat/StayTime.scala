...
class HourSceneStaytime(spark: SparkSession, dateStr: String, hour: String) extends Serializable {
  def insert2db(): Unit = {
    ...
    spark.sql("SELECT ak,scene,sum(time) total_time FROM " +
      ...
      "FROM stayTime_hour WHERE ev !='event' GROUP BY ak,scene,at) group by ak,scene").distinct().createTempView("scene_hour")
    //单个场景值次均停留时长
...
class HourTrendStaytime(spark: SparkSession, dateStr: String, hour: String) extends Serializable {
  ...
  def insert2db(): Unit = {
    ...
    spark.sql("SELECT ak,sum(time) total_time FROM (SELECT ak,cast(max(st)/1000 as int)-cast(min(st)/1000 as int) time FROM " +
      "stayTime_hour WHERE ev !='event' GROUP BY ak,at) group by ak").distinct().createTempView("trendtime")
...
class DaySceneStaytime(spark: SparkSession, dateStr: String, timeStr: String) extends Serializable {
  ...
  def insert2db(): Unit = {
    ...
    //单个场景值  天  停留时长
    spark.sql("SELECT ak,scene,sum(time) total_time FROM (SELECT ak,scene,cast(max(st)/1000 as int)-cast(min(st)/1000 as int) time FROM stayTime_day WHERE ev!='event' GROUP BY ak,scene,at) group by ak,scene").createTempView("scene_day")
...
class DayTrendStaytime(spark: SparkSession, dateStr: String) extends Serializable {
  ...
  def insert2db(): Unit = {
    ...
    spark.sql("SELECT ak, sum(time) total_time, UNIX_TIMESTAMP(now()) FROM (SELECT ak,cast(max(st)/1000 as int)-cast(min(st)/1000 as int) time " +
      "FROM stayTime_day WHERE ev !='event' GROUP BY ak,at) group by ak").distinct().createTempView("trendtime1")
    //趋势分析tian打开次数
...
class DRHourSceneStaytime(spark: SparkSession, dateStr: String, hour: String) extends Serializable {
  ...
  def insert2db(): Unit = {
   try{
     ...
     //单个场景值的 小时 总停留时长
     spark.sql("SELECT ak,scene,sum(time) total_time FROM (SELECT ak,scene,MAX(dr/1000) time FROM drstayTime_hour WHERE ev !='event' GROUP BY ak,scene,at) group by ak,scene").createTempView("drscene_hour")
     //单个仓净值的次均停留时长
  ...
  def insert2db(): Unit = {
    ...
    try{
      spark.sql("SELECT ak,sum(time) total_time FROM (SELECT ak,max(dr/1000) time FROM drstayTime_hour " +
        "WHERE ev !='event' GROUP BY ak,at) group by ak").distinct().createTempView("drtrendtime")
      //趋势分析打开次数
  ...
  def insert2db(): Unit = {
    try{
      ...
      spark.sql("SELECT ak,scene,sum(time) total_time FROM (SELECT ak,scene,MAX(dr/1000) time " +
        "FROM drstayTime_day WHERE ev !='event' GROUP BY ak,scene,at) group by ak,scene").createTempView("drscene_day")
      //单个仓净值的次均停留时长
  ...
  def insert2db(): Unit = {
    try{
      ...
      spark.sql("SELECT ak,sum(time) total_time FROM (SELECT ak,max(dr/1000) time FROM drstayTime_day " +
        "WHERE ev !='event' GROUP BY ak,at) group by ak").distinct().createTempView("drtrendtime2")
