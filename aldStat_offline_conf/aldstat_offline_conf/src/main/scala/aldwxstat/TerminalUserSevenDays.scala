...
object TerminalUserSevenDays {
  def main(args: Array[String]): Unit = {
    ...
    //val df = ArgsTool.getTencentLogs(args, sparkseesion, ConfigurationUtil.getProperty("tencent.parquet"))
    ...
    for (terminal <- terminal_sum) {
      ...
      visnum.foreachPartition((rows: Iterator[Row]) => {
        ...
        val sqlText = s"insert into aldstat_7days_terminal_analysis (app_key,day,type_value,visitor_count,type,update_at)" +
          s"values (?,?,?,?,?,?) ON " +
