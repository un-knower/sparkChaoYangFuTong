...
/**
  ...
  * 汇总统计
  */
object EntrancePageDetail_30day {
...
//    val logs_df = ArgsTool.getLogs(args, spark, ConfigurationUtil.getProperty("tongji.parquet"))
  ...
  def statData(spark:SparkSession) = {
    ...
    spark.sql(
      """
        |SELECT * FROM
        |(SELECT ROW_NUMBER()over(partition by at ORDER BY st ASC) rowId, ak, uu, at, pp,dr from logs_page) as demo
  ...
  def batchInsert(spark:SparkSession,args: Array[String])={
    ...
    result.foreachPartition(rows =>{
      ...
      val sql =
        """
          |INSERT INTO aldstat_30days_single_entrance_page
