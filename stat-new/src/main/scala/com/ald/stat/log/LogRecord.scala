...
import com.ald.stat.utils.{ComputeTimeUtils, GlobalConstants}
...
object LogRecord extends Serializable {
  ...
  def line2Bean(line: String): LogRecord = {
    try {
      ...
      if (logRecord != null) {
        if (StringUtils.isNotBlank(logRecord.wsr) && logRecord.wsr.isInstanceOf[String] && logRecord.wsr.startsWith("{") && logRecord.wsr.endsWith("}")) {
          ...
          //从wsr中获取path字段
          if (StringUtils.isBlank(logRecord.path)) {
